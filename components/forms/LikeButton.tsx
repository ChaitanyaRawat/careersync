...
const LikeButton = ({ threadId, currentUserId, isLiked }: Props) => {
    ...
    return (
        <>
            <button className={` font-bold py-2 px-4 border-2 ${likeState ? 'border-primary-500' : 'border-black'}   rounded flex items-center gap-2 w-1/2 justify-center`} onClick={() => handleLike()}>
                <Image
                ...
                />
                <span className={`text-lg ${likeState ? 'text-primary-500' : 'text-black'}`}>{likeState ? 'Liked' : 'Like'}</span>
            </button>
        </>
    )
}
