...
function PostCareerpost({ userId }: Props) {
    ...
    const form = useForm<z.infer<typeof CareerpostValidation>>({
        resolver: zodResolver(CareerpostValidation),
        defaultValues: {
            careerpost: "",
            accountId: userId,
            attachment: "",
    ...
    const onSubmit = async (values: z.infer<typeof CareerpostValidation>) => {
        setisLoading(true)
        ...
        await createCareerpost({
            text: values.careerpost,
            author: userId,
            companyId: organization ? organization.id : null,
            path: pathname,
            tags: tags,
            urls: urls,
